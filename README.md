...
매일 14:00 KST에 AI 뉴스를 모아 Claude API로 선별·작성하고, GitHub Pages에 올린 뒤
카카오톡 "나에게 보내기"로 요약 + 링크를 보냅니다. 2주마다 🔥 반응을 분석해 카카오톡으로
요약해 줍니다.

## 구성

- `pipeline/` — 수집(RSS) → 선별/작성(Claude API) → 렌더링 → 카카오톡 전송 파이썬 스크립트
...
| 이름 | 용도 |
|---|---|
| `ANTHROPIC_API_KEY` | Claude API 호출 (선별/작성, 웹 검색, 2주 분석 요약) |
