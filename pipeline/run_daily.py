...
수집 -> 선별/작성(Claude) -> 페이지 렌더링 -> 카카오톡 전송까지 한 번에 수행한다.
...
from send_kakao import send_briefing
...
def main() -> None:
    ...
    print("[run_daily] 4/4 카카오톡 전송 중...")
    result = send_briefing(data["date"], data["kakao_text"])
    print(f"[run_daily] 카카오톡 전송 완료: {result}")
